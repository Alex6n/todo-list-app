...
import { initiateEdit, instantPriority, instantUpdateTask } from '../../store/editSlice';
        ...
        <div className='flex flex-col items-center'>
          <div className="mb-5">
            <div className="mt-3 flex gap-1 text-sm font-bold">
              <button href="#" onClick={() => dispatch(instantPriority(index))} className={`filter badge ${edit.Priority.toLowerCase()}`} >
                {edit.Priority}</button>
            </div>
          </div>

          <div className='flex flex-col justify-center'>
            <button href="#" onClick={applyChangesHandler} className="card-action cmplt"><ImCheckmark className="mr-1" /> Apply Changes</button>
          </div>
